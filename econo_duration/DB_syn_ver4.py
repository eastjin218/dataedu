...
def tts_process(idx, texts, speaker, num):
    ...
    for i in texts:
        ...
        if count <70:
            i = i.split('-')[0]
            p_i ='%s%s%s\n'%(i,pad,i[:2])
            audio = synthesizer.synthesize(texts=[p_i],base_path=output, speaker_ids=[0],attention_trim=True,num=num)[0]
            ...
            audio = synthesizer.synthesize(texts=[i],base_path=output,speaker_ids=[0],attention_trim=True,num=num)[0]
    ...
    if str(idx) in a_tmp:
        ...
        state='R'
        audio='Null'
    curs.execute("UPDATE news_title SET TTS_YN=%s, state=%s,  audio=%s WHERE n_idx=%s",(tts, state, audio, str(idx)))
    con.commit()
...
for i,j in data.iterrows():
    ta = []
    idx=j.n_idx
    for num, text in enumerate (j.split_contents):
        ta.append((idx,text,'2',num))
    pool=mp.Pool(8)
    pool.starmap(tts_process, ta)
    pool.close()
